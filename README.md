...
<p align="center">
  <img src="docs/demo.gif" alt="李记·TEA 演示:分类 → 绿茶列表(价格排序)→ 商品详情 → 商品评价 → 购物车 → 我的 → 茶席" width="300" />
</p>

完整高清录屏(含测试标注):[docs/demo.mp4](docs/demo.mp4)

演示流程:分类 → 绿茶列表(价格升序排序)→ 商品详情(产品介绍 + 参数表 + 双按钮)→ 商品评价(评分分布 + 买家图)→ 购物车(数量步进 + 实时合计)→ 我的(深绿头部)→ 茶席(标签栏 + 精选图 + 美学网格)。
